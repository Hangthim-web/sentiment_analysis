...
def analyze_sentiment_using_BoW_and_NB(request,user_input):
    try:
        ...
        if probability_negative > 0.65:

            if request.user.is_authenticated:
                print(f"Authenticated User: {request.user}")
                request.user.isHate = True
                request.user.save()
            else:
                print("User is not authenticated. Cannot update isHate field.")
...
result = analyze_sentiment_using_BoW_and_NB(request,user_input)
print("\n" + "=" * 50 + "\n")
